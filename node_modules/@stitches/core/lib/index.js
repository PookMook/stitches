...
exports.createCss = exports.createTokens = exports.hotReloadingCache = void 0;
...
exports.hotReloadingCache = new Map();
...
var resolveTokens = function (cssProp, value, tokens) {
    var token = utils_1.cssPropToToken[cssProp];
    var tokenValue;
    if (token) {
...
var processStyleObject = function (obj, config, valueMiddleware, currentNestingPath, shouldHandleUtils, shouldHandleSpecificityProps) {
    ...
    for (var _i = 0, _a = Object.keys(obj); _i < _a.length; _i++) {
        ...
        var isSpecificityProp = shouldHandleSpecificityProps && !isUtilProp && key in utils_1.specificityProps;
        /** Nested styles: */
        if (typeof val === "object" && !isSpecificityProp && !isUtilProp) {
            ...
            processStyleObject(val, config, valueMiddleware, tslib_1.__spreadArrays(currentNestingPath, [
                key,
            ]));
            continue;
        }
        ...
        if (isUtilProp) {
            ...
            processStyleObject(resolvedUtils, config, valueMiddleware, tslib_1.__spreadArrays(currentNestingPath), false);
            continue;
        }
        /** Specificity Props: */
        ...
        if (isSpecificityProp) {
            var resolvedSpecificityProps = utils_1.specificityProps[key](config.tokens, val);
            processStyleObject(resolvedSpecificityProps, config, valueMiddleware, tslib_1.__spreadArrays(currentNestingPath), false, false);
            continue;
        }
        if (typeof val === "number") {
            ...
            "" + (unitless_1.unitlessKeys[key] ? val : val + "px"), currentNestingPath);
        }
...
var resolveBreakpointAndSelectorAndInlineMedia = function (nestingPath, config) {
    return nestingPath.reduce(function (acc, breakpointOrSelector, i) {
        ...
        if (breakpointOrSelector in config.breakpoints ||
            breakpointOrSelector === utils_1.MAIN_BREAKPOINT_ID) {
            if (acc.breakpoint !== utils_1.MAIN_BREAKPOINT_ID) {
        ...
        breakpoint: utils_1.MAIN_BREAKPOINT_ID,
        nestingPath: "",
        inlineMediaQueries: [],
    });
};
...
var createCssRule = function (breakpoints, atom, className) {
    ...
    return atom.breakpoint !== utils_1.MAIN_BREAKPOINT_ID
...
var composeIntoMap = function (map, atoms) {
    ...
    for (; i >= 0; i--) {
        ...
        if (item && item[types_1.ATOM] && "atoms" in item) {
            composeIntoMap(map, item.atoms);
        }
        else if (item && item[types_1.ATOM]) {
            if (!map.has(item.id)) {
...
exports.createCss = function (_config, env) {
    ...
    constants_1.tokenTypes.forEach(function (tokenType) { return (config.tokens[tokenType] = config.tokens[tokenType] || {}); });
    ...
    var _a = env
        ? utils_1.getVendorPrefixAndProps(env)
        : { vendorPrefix: "-node-", vendorProps: [] }, vendorPrefix = _a.vendorPrefix, vendorProps = _a.vendorProps;
    if (env && exports.hotReloadingCache.has(prefix)) {
        var instance = exports.hotReloadingCache.get(prefix);
        instance.dispose();
    }
    ...
    var cssClassnameProvider = function (atom) {
        ...
        var hash = utils_1.hashString("" + (atom.breakpoint || "") + atom.cssHyphenProp.replace(/-(moz|webkit|ms)-/, "") + (atom.selector || "") + (((_a = atom.inlineMediaQueries) === null || _a === void 0 ? void 0 : _a.join("")) || "") + atom.value);
    ...
    var _b = utils_1.createSheets(env, config.breakpoints), tags = _b.tags, sheets = _b.sheets;
    ...
    var keyframesToString = createKeyframesToString(sheets[utils_1.MAIN_BREAKPOINT_ID]);
    ...
    var cssInstance = (function () {
        ...
        for (var x = 0; x < definitions.length; x++) {
            ...
            if (typeof definitions[x] === "string" || definitions[x][types_1.ATOM]) {
    ...
    cssInstance.keyframes = function (definition) {
        ...
        var hash = utils_1.hashString(cssRule);
    ...
    cssInstance.getStyles = function (cb) {
        ...
        keyframesToString = createKeyframesToString(sheets[utils_1.MAIN_BREAKPOINT_ID]);
        ...
        return {
            ...
            styles: Object.keys(breakpoints).reduce(function (aggr, key) {
                ...
                "/* STITCHES */\n" + sheets[utils_1.MAIN_BREAKPOINT_ID].cssRules.join("\n"),
            ]),
        };
    };
    if (env) {
        exports.hotReloadingCache.set(prefix, cssInstance);
    }
    return cssInstance;
};
